...
async def live_update():
    ...
    if (hunter.lower() in HUNTERS) and (tdy == date) and (bug_state == "new") and latest_report != old_feed['items'][0]:
        embed = discord.Embed(
    ...
    elif (hunter.lower() in HUNTERS) and (tdy == date) and (bug_state == "accepted") and latest_report != old_feed['items'][0]:
        embed = discord.Embed(
        ...
        await channel.send(embed=embed)
    elif (hunter.lower() in HUNTERS) and (tdy == date) and (bug_state == "resolved") and latest_report != old_feed['items'][0]:
        embed = discord.Embed(
